...
def normalize_report_severity(val: Any) -> str:
    """Normalize severity values to standard CRITICAL, HIGH, MEDIUM, LOW, INFORMATIONAL."""
    if not val:
        return "HIGH"
...
def generate_docx_report(
    ...
    # Normalize priority across all findings to eliminate raw priority codes (e.g. P1, P2, P3)
    for f in source_findings:
        prio = normalize_report_severity(f.get("priority") or f.get("severity"))
        f["priority"] = prio
        f["severity"] = prio
    ...
    # Preserve authentic finding IDs to guarantee strict traceability (fix branch / commit / PR matching)
    for idx, f in enumerate(active_findings):
        auth_id = f.get("vuln_id") or f.get("source_finding_id")
        if not auth_id:
            auth_id = f"VULN-{idx + 1:03d}"
            f["vuln_id"] = auth_id
        f["report_vuln_id"] = auth_id
    ...
    for idx, f in enumerate(active_findings):
        ...
        def add_subheading(title_text: str, color_val=COLOR_NAVY):
            p_sub = doc.add_paragraph()
            p_sub.paragraph_format.space_before = Pt(6)
            p_sub.paragraph_format.space_after = Pt(2)
            p_sub.paragraph_format.keep_with_next = True
            r_s = p_sub.add_run(title_text)
            r_s.bold = True
            r_s.font.name = "Calibri"
            r_s.font.size = Pt(10)
            r_s.font.color.rgb = color_val
            return p_sub

        # A. Executive Description
        add_subheading("A. Executive Description", COLOR_NAVY)
        exec_desc = f.get("description") or f.get("executive_summary") or f"Tracegate security assessment identified a confirmed {finding_title} requiring remediation to uphold application security boundaries."
        p_ed = doc.add_paragraph(exec_desc)
        p_ed.paragraph_format.space_after = Pt(6)

        # B. Technical Observation
        add_subheading("B. Technical Observation", COLOR_NAVY)
        obs_text = f.get("observation") or f.get("description") or "Technical observation not provided."
        p_obs = doc.add_paragraph(obs_text)
        p_obs.paragraph_format.space_after = Pt(6)

        # C. Affected Asset & Component
        add_subheading("C. Affected Asset & Component", COLOR_NAVY)
        p_comp = doc.add_paragraph()
        p_comp.add_run(f"• Target Endpoint / URL: {f.get('affected_url') or '/api'}\n")
        p_comp.add_run(f"• Affected Subsystem / Component: {f.get('affected_component') or 'Web Application'}\n")
        p_comp.add_run(f"• Provenance / Source ID: {f.get('source_finding_id') or vuln_id} ({src_label})")
        p_comp.paragraph_format.space_after = Pt(6)

        # D. Security Impact
        add_subheading("D. Security Impact", COLOR_NAVY)
        sec_imp = f.get("security_impact")
        if not sec_imp:
            full_imp = f.get("impact") or ""
            sec_imp = full_imp.split("Business Impact:")[0].replace("Security Impact:", "").strip() if "Business Impact:" in full_imp else full_imp
        p_si = doc.add_paragraph(sec_imp or f"Exploitation of {finding_title} allows adversaries to compromise confidentiality, integrity, or availability of target application workflows.")
        p_si.paragraph_format.space_after = Pt(6)

        # E. Business Impact
        add_subheading("E. Business Impact", COLOR_NAVY)
        biz_imp = f.get("business_impact")
        if not biz_imp and f.get("impact") and "Business Impact:" in f.get("impact"):
            biz_imp = f.get("impact").split("Business Impact:", 1)[1].strip()
        p_bi = doc.add_paragraph(biz_imp or "Unremediated security vulnerabilities expose the business to regulatory non-compliance fines, unauthorized operations, customer data breach exposure, and reputational impairment.")
        p_bi.paragraph_format.space_after = Pt(6)

        # F. Step-by-Step Reproduction Procedure
        add_subheading("F. Step-by-Step Reproduction Procedure", COLOR_NAVY)
        repro = f.get("reproduction_steps") or f.get("steps_to_reproduce") or f.get("testing_notes") or "1. Access target application.\n2. Submit targeted test payload.\n3. Validate anomalous response."
        p_rep = doc.add_paragraph(repro)
        p_rep.paragraph_format.space_after = Pt(6)

        # G. Proof of Concept (PoC) Request / Payload Snippet
        add_subheading("G. Proof of Concept (PoC) Request / Payload Snippet", COLOR_NAVY)
        ...
        if evidence_list:
            ...
            r_evh = p_ev_head.add_run("H. Attached Proof-of-Concept Evidence")
            r_evh.bold = True
        ...
        add_subheading("I. Root Cause Analysis", COLOR_NAVY)
        root_text = f.get("root_cause") or tmpl.get("root_cause") or "The underlying defect stems from missing server-side authorization boundaries and inadequate input parameter validation prior to execution."
        p_rc = doc.add_paragraph(root_text)
        p_rc.paragraph_format.space_after = Pt(6)

        # J. Recommended Technical Remediation
        add_subheading("J. Recommended Technical Remediation", COLOR_LOW)
        ...
        add_subheading("K. Defense-in-Depth Architectural Mitigation", COLOR_NAVY)
        ...
        p_mit.paragraph_format.space_after = Pt(6)

        # L. Detection & Monitoring Guidance
        add_subheading("L. Detection & Monitoring Guidance", COLOR_NAVY)
        det_text = f.get("detection_monitoring") or tmpl.get("detection_monitoring") or f"Configure application logging to capture anomalous parameter values and audit authorization rejections targeting {vuln_id}. Set SIEM threshold alerts for repeated access boundary violations."
        p_det = doc.add_paragraph(det_text)
        p_det.paragraph_format.space_after = Pt(6)

        # M. Developer Validation Guidance
        add_subheading("M. Developer Validation Guidance", COLOR_NAVY)
        val_text = f.get("developer_validation") or tmpl.get("developer_validation") or f"Developers should execute automated unit and integration tests verifying that unauthorized attempts against {f.get('affected_url') or '/api'} consistently return HTTP 403 Forbidden or 400 Bad Request."
        p_val = doc.add_paragraph(val_text)
        p_val.paragraph_format.space_after = Pt(6)

        # N. AI AutoFix Remediation Tracking & GitHub Audit (Strict Finding Isolation)
        add_subheading("N. AI AutoFix Remediation Tracking & GitHub Audit", COLOR_NAVY)

        # Strict isolation: ensure Git branch / commit / PR belongs to this specific vuln_id
        safe_branch = f.get("github_branch")
        safe_commit = f.get("github_commit")
        safe_pr = f.get("github_pr")
        vuln_clean = re.sub(r'[^a-zA-Z0-9]', '', vuln_id.upper())
        if safe_branch:
            b_match = re.search(r'(?i)VULN-?(\d+)', safe_branch)
            if b_match and vuln_clean.startswith("VULN"):
                cur_num = re.search(r'\d+', vuln_clean)
                if cur_num and int(b_match.group(1)) != int(cur_num.group(0)):
                    # Cross-contamination prevented! Suppress foreign branch.
                    safe_branch = None
                    safe_commit = None
                    safe_pr = None
        ...
        if safe_branch:
            p_life.add_run(f"• Dedicated Fix Branch: {safe_branch}\n")
        if safe_commit:
            p_life.add_run(f"• Remediation Commit SHA: {safe_commit}\n")
        if safe_pr:
            p_life.add_run(f"• Pull Request: {safe_pr}\n")
        if not (safe_branch or safe_commit or safe_pr):
            p_life.add_run("• GitHub Code Automation: Awaiting automated fix generation or direct repository merge.\n")
        p_life.paragraph_format.space_after = Pt(6)

        # O. Retest & Verification Lifecycle
        add_subheading("O. Retest & Verification Lifecycle", COLOR_NAVY)
        p_rt = doc.add_paragraph()
        retest_st = f.get("retest_status") or "PENDING"
        p_rt.add_run(f"• Retest Verification Status: {retest_st}\n")
        if f.get("retest_notes"):
            p_rt.add_run(f"• Retest Verification Notes: {f.get('retest_notes')}\n")
        if f.get("retested_at"):
            p_rt.add_run(f"• Verification Timestamp: {f.get('retested_at')}\n")
        if not f.get("retest_notes") and not f.get("retested_at"):
            p_rt.add_run("• Retest Verification Notes: Finding is pending re-assessment by testing engineers upon remediation deployment.\n")
        p_rt.paragraph_format.space_after = Pt(14)
