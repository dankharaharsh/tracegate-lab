...
def ai_fix_retest_endpoint(finding_id: str, req: AIFixRetestRequest, authorization: Optional[str] = Header(None)):
    ...
    try:
        if res["result"] == "PASSED":
            f_obj = res.get("finding") or finding
            p_id = f_obj.get("project_id") if isinstance(f_obj, dict) else None
            if p_id:
                elig = check_assessment_certificate_eligibility(p_id)
                cert_eligible = bool(elig.get("eligible"))
                if cert_eligible:
                    gen_res = generate_assessment_certificate(p_id)
                    if gen_res.get("success"):
                        cert_data = gen_res.get("certificate")
    except Exception as ce:
        logger.warning(f"Certificate evaluation error post-retest (non-blocking): {ce}")
...
def project_certificate_status_endpoint(project_id: str):
    """
...
def project_certificate_generate_endpoint(project_id: str):
    """
...
def project_certificates_list_endpoint(project_id: str):
    """
...
def get_certificate_endpoint(certificate_id: str):
    """
...
def download_certificate_endpoint(certificate_id: str, format: Optional[str] = Query("pdf")):
    """
    Download certificate package in native PDF or Microsoft Word DOCX format.
    """
    cert = get_certificate_by_id(certificate_id)
    if not cert:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Certificate record not found.")
...
def dismiss_certificate_notice_endpoint(certificate_id: str):
    """
