                            ...
                            <tr>
                                ...
                                <td>
                                    <div style="display: flex; gap: 6px;">
                                        <a href="/api/projects/${projId}/reports/${r.id}/download?format=docx" class="btn btn-outline-primary btn-sm" download title="Download Microsoft Word Document">
                                            📥 DOCX
                                        </a>
                                        <a href="/api/projects/${projId}/reports/${r.id}/download?format=pdf" class="btn btn-outline-danger btn-sm" download title="Download Native PDF Document">
                                            📄 PDF
                                        </a>
                                    </div>
                                </td>
...
`;
    ...
    async function dismissCertificateNotice(certId) {
        ...
        try {
            ...
            await fetch(`/api/certificates/${certId}/dismiss-notice`, { method: "POST" });
        } catch (e) {
            ...
            if (data.status === "GENERATED" && data.certificate) {
                ...
                if (contentArea) {
                    ...
                    document.getElementById("btnWsDownloadCert")?.addEventListener("click", () => {
                        window.open(`/api/certificates/${cert.certificate_id}/download?format=pdf`, "_blank");
                    });
                }
                ...
                if (contentArea) {
                    contentArea.innerHTML = `
                        <div style="background: var(--bg-subtle); border: 1px dashed var(--border-default); border-radius: 8px; padding: 14px 16px; font-size: 0.85rem; color: var(--text-secondary); line-height: 1.5;">
                            ...
                            <div>${escapeHtml(data.reason || "Assessment has in-scope findings pending remediation validation.")}</div>
                        </div>
                    `;
                }
    ...
    if (btnDownloadCertFromCompletion) {
        btnDownloadCertFromCompletion.addEventListener("click", () => {
            if (activeCertificateData) {
                window.open(`/api/certificates/${activeCertificateData.certificate_id}/download?format=pdf`, "_blank");
            }
        });
    }
    ...
    if (btnDownloadCertDocxModal) {
        btnDownloadCertDocxModal.addEventListener("click", () => {
            if (activeCertificateData) {
                window.open(`/api/certificates/${activeCertificateData.certificate_id}/download?format=docx`, "_blank");
            }
        });
    }
    if (btnVerifyCertOnlineModal) {
        btnVerifyCertOnlineModal.addEventListener("click", () => {
            if (activeCertificateData) {
                window.open(`/certificate/verify/${activeCertificateData.certificate_id}`, "_blank");
            }
        });
    }
