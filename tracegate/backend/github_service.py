...
4. Safe dedicated fix branch creation (tracegate/fix/{vuln_id}).
5. Structured git commit creation.
6. Safe automated validation check (lint/syntax/tests).
7. Formal Pull Request generation with comprehensive security description.
"""
...
# Vulnerability-to-Source-Code Fix Mapping
CODE_FIX_PATTERNS = {
    "CWE-639": {
...
def save_github_token(user_id: str, token: str, username: Optional[str] = None) -> GitHubStatusResponse:
    ...
    save_user_github_config(user_id, token, resolved_username)
    preview = f"{token[:4]}...{token[-4:]}" if len(token) > 8 else "***"
    return GitHubStatusResponse(
        connected=True,
...
def analyze_finding_code(
    ...
    """
    Analyzes vulnerable code corresponding to a finding, identifies root cause,
    and generates unified git diff preview.
    """
    ...
    file_path = finding.get("affected_component") or matching_pattern["file_path"]
    original_code = matching_pattern["original"]
...
def apply_finding_fix(
    ...
    """
    Creates a dedicated fix branch (tracegate/fix/{vuln_id}), applies patch,
    creates structured commit, and runs automated simulation validation.
    """
    vuln_id = finding.get("vuln_id") or finding.get("id") or "VULN-001"
