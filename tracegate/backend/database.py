...
def save_imported_findings(
    ...
    for idx, cand in enumerate(candidate_findings):
        ...
        """, (
            ...
            imp, repro, remed, mitig, status_val, "NOT_STARTED",
            "IMPORTED_REPORT", source_doc_id, source_doc_name, "PENDING", "", now
        ))
...
def get_ai_fix_for_finding(finding_id: str) -> Optional[Dict[str, Any]]:
    conn = get_db_connection()
    cursor = conn.cursor()
    cursor.execute("SELECT * FROM ai_fixes WHERE finding_id = ? ORDER BY updated_at DESC LIMIT 1", (finding_id,))
    row = cursor.fetchone()
    conn.close()
    if not row:
        return None
...
def get_ai_fix_by_pr_number(pr_number: int, repo: Optional[str] = None) -> Optional[Dict[str, Any]]:
    ...
    cursor = conn.cursor()
    if repo:
        cursor.execute("SELECT * FROM ai_fixes WHERE pr_number = ? AND repository = ? ORDER BY updated_at DESC LIMIT 1", (pr_number, repo))
    else:
        cursor.execute("SELECT * FROM ai_fixes WHERE pr_number = ? ORDER BY updated_at DESC LIMIT 1", (pr_number,))
    row = cursor.fetchone()
    conn.close()
    if not row:
        return None
