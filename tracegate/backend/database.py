...
def add_custom_checklist_item(project_id: str, data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    cursor.execute("""
    INSERT INTO checklist_items (id, project_id, checklist_id, test_id, name, priority, reason, testing_objective, cwe, source, status, sort_order, created_at)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, 'USER', 'NOT_TESTED', 99, ?)
    """, (
        item_id,
        project_id,
        None,
        item_id,
        data.get("name", "Custom Security Test"),
        data.get("priority", "HIGH"),
        data.get("reason", "Learner-defined custom test procedure."),
        data.get("testing_objective", ""),
        data.get("cwe", ""),
        now
    ))
    cursor.execute("UPDATE projects SET updated_at = ? WHERE id = ?", (now, project_id))
...
def save_finding(project_id: str, item_id: Optional[str], finding_data: Dict[str, Any]) -> Dict[str, Any]:
