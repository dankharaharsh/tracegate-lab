...
`;
    ...
    async function autoDetectSourceFile() {
        ...
        try {
            showToast("Scanning repository tree for vulnerable source file...", "info");
            const res = await fetch("/api/ai-fix/discover-file", {
                method: "POST",
                headers: { "Content-Type": "application/json" },
                body: JSON.stringify({ finding_id: findingId, repo, branch })
            });
            ...
            const badge = document.getElementById("wsFileDetectionBadge");
            const reasonEl = document.getElementById("wsFileDetectionReason");

            if (data.file_path) {
                if (fileInput) fileInput.value = data.file_path;
                if (badge) {
                    badge.style.display = "inline-block";
                    badge.textContent = `Auto-matched (${Math.round(data.confidence * 100)}%)`;
                }
                if (reasonEl) {
                    reasonEl.style.display = "block";
                    reasonEl.textContent = data.reason;
                }
                showToast(`✓ Auto-detected source file: ${data.file_path}`, "success");
            } else {
                if (reasonEl) {
                    reasonEl.style.display = "block";
                    reasonEl.textContent = data.reason || "Unable to confidently locate source file. Please select manually.";
                }
                showToast(data.reason || "Could not automatically locate file.", "warning");
            }
        } catch (e) {
    ...
    function renderRepoTreeList(items) {
        ...
        items.forEach(item => {
            ...
            div.className = "repo-tree-item";

            div.innerHTML = `
                <span style="color: var(--text-primary);">${item.path}</span>
                <span style="font-size: 0.72rem; color: var(--text-muted); background: var(--bg-subtle); padding: 2px 6px; border-radius: 4px;">${item.size ? item.size + ' B' : 'blob'}</span>
            `;

            div.addEventListener("click", () => {
                const fileInput = document.getElementById("wsAutofixFileInput");
                if (fileInput) fileInput.value = item.path;
                closeModal("modalBrowseRepoFiles");
                showToast(`Selected file: ${item.path}`, "info");
            });
    ...
    async function executeCodeFixAnalysis(repo, branch, findingId, isWorkspace = true) {
        ...
        try {
            showToast("Analyzing vulnerable source code & generating unified diff...", "info");
            const res = await fetch("/api/ai-fix/analyze", {
                method: "POST",
                headers: { "Content-Type": "application/json" },
                body: JSON.stringify({
                    repo: repoToUse,
            ...
            if (shaBadge) {
                shaBadge.textContent = data.file_sha ? `SHA: ${data.file_sha.substring(0, 7)}` : "";
            }
            ...
            // Populate discrete changes list
            if (changesList) {
    ...
    if (btnSubmitRevisionRequest) {
        btnSubmitRevisionRequest.addEventListener("click", async () => {
            ...
            try {
                ...
                if (afterCodeContainer) afterCodeContainer.textContent = revisedData.after_code || revisedData.proposed_code || "";
                ...
                if (diffContainer) {
                    diffContainer.innerHTML = "";
                    (revisedData.diff_unified || "").split("\n").forEach(line => {
                        const span = document.createElement("span");
                        ...
                        else if (line.startsWith("@@")) span.className = "diff-line-info";
                        span.textContent = line + "\n";
                        diffContainer.appendChild(span);
                    });
                }
    ...
    if (btnWsFailRetest) {
        btnWsFailRetest.addEventListener("click", async () => {
            ...
            try {
                ...
                if (retestSuccessBadge) {
                    retestSuccessBadge.style.display = "block";
                    retestSuccessBadge.innerHTML = `<span style="color: #b91c1c;">✗ Vulnerability still reproducible. Finding transitioned: <strong>REOPENED</strong>. You may request a revision or regenerate fix.</span>`;
                }
    ...
    if (wsAutofixFindingSelect) {
        wsAutofixFindingSelect.addEventListener("change", (e) => {
            ...
            const proj = getActiveProject();
            const f = proj?.findings?.find(item => item.id === fid);
            if (f && (f.affected_component || f.affected_endpoint)) {
                const fileInput = document.getElementById("wsAutofixFileInput");
                if (fileInput && !fileInput.value) {
                    fileInput.value = f.affected_component || "";
                }
            }
    ...
    if (btnWsRejectFix) {
        btnWsRejectFix.addEventListener("click", () => {
            ...
            if (diffWrap) diffWrap.style.display = "none";
            showToast("Fix proposal rejected. Record preserved in audit log.", "info");
        });
    }
