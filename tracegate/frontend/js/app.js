...
`;
        ...
        try {
            ...
            if (changesList) {
                ...
                if (changes.length > 0) {
                    ...
                    const li = document.createElement("li");
                    li.textContent = "Applied defense-in-depth authorization check and input validation.";
                    changesList.appendChild(li);
                }
            }
    ...
    async function executeApplyFix(isWorkspace = true) {
        ...
        try {
            ...
            if (res.status === 409) {
                ...
                throw new Error(errData.detail || "The source file has changed since this fix was generated. Please regenerate the fix before applying it.");
            }

            if (!res.ok) {
                const errData = await res.json().catch(() => ({}));
                throw new Error(errData.detail || "Failed to apply fix.");
            }
            ...
            // Reset interactive retesting checkbox
            const chkRetest = document.getElementById("chkRetestFinding");
            if (chkRetest) chkRetest.checked = false;
            const retestBadge = document.getElementById("retestSuccessBadge");
            if (retestBadge) retestBadge.style.display = "none";
            ...
            if (proj && proj.findings) {
                ...
                if (f) {
                    ...
                    f.ai_fix = {
                        status: "Fix Applied",
                        branch_name: fixResult.branch_name,
                        commit_sha: fixResult.commit_sha,
                        file_path: data.file_path
                    };
                    saveProjects();
                    renderWorkspaceFindings();
        ...
        try {
            ...
            const res = await fetch("/api/github/create-pr", {
                ...
                body: JSON.stringify({
                    ...
                    title: `Security Fix (${fixResult.branch_name}): Remediate finding ${data.finding_id}`,
            ...
            if (proj && proj.findings) {
                ...
                if (f) {
                    ...
                    if (!f.ai_fix) f.ai_fix = {};
                    f.ai_fix.status = "PR Created";
                    f.ai_fix.pull_request_url = prResult.pr_url;
                    f.ai_fix.pull_request_number = prResult.pr_number;
                    saveProjects();
                    renderWorkspaceFindings();
                }
            }
    ...
    // Interactive Retesting & Validation Checklist Handler
    const chkRetestFinding = document.getElementById("chkRetestFinding");
    if (chkRetestFinding) {
        chkRetestFinding.addEventListener("change", (e) => {
            const retestBadge = document.getElementById("retestSuccessBadge");
            const data = window._currentAnalysisData;
            ...
            if (e.target.checked) {
                if (retestBadge) retestBadge.style.display = "block";
                const proj = getActiveProject();
                if (proj && proj.findings) {
                    const f = proj.findings.find(item => item.id === data.finding_id);
                    if (f) {
                        ...
                        if (!f.ai_fix) f.ai_fix = {};
                        f.ai_fix.status = "Resolved";
                        f.ai_fix.validated = true;
                        saveProjects();
                        renderWorkspaceFindings();
                    }
                }
